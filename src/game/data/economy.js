export const RESOURCE_TABLE = {
  tree: {
    name: 'Arbre',
    icon: '/assets/Weapons/Wood/Wood.png',
    drops: [
      { material: 'wood', min: 2, max: 4, chance: 1 },
      { material: 'resin', min: 1, max: 2, chance: 0.5 },
    ],
  },
  ore: {
    name: 'Veine de minerai',
    icon: '/assets/Weapons/Bone/Bone.png',
    drops: [
      { material: 'ore', min: 2, max: 4, chance: 1 },
      { material: 'obsidian_fragment', min: 1, max: 2, chance: 0.4 },
    ],
  },
  herb: {
    name: 'Herbes de brume',
    icon: '/assets/Weapons/Hands/Hands.png',
    drops: [
      { material: 'herb', min: 2, max: 3, chance: 1 },
      { material: 'ether_drop', min: 1, max: 2, chance: 0.35 },
    ],
  },
}

export const MATERIAL_LABELS = {
  wood: 'Bois brut',
  resin: 'Résine vive',
  ore: 'Minerai brut',
  obsidian_fragment: 'Fragment d\'obsidienne',
  herb: 'Herbe de brume',
  ether_drop: 'Goutte d\'ether',
  bone_dust: 'Poussière osseuse',
  boss_shard: 'Eclat de boss',
  misty_heart: 'Cœur brumeux',
  stability_shard: 'Éclat de stabilité',
  spirit_chisel: 'Ciseau des esprits',
}

export const RECIPES = [
  {
    id: 'recipe_major_potion',
    name: 'Potion majeure',
    rarity: 'uncommon',
    description: 'Soin rapide en combat.',
    materials: { herb: 4, resin: 1 },
    result: {
      kind: 'consumable',
      name: 'Potion majeure',
      effect: 'heal_80',
      quantity: 1,
    },
  },
  {
    id: 'recipe_hunter_bow',
    name: 'Arc du traqueur',
    rarity: 'rare',
    description: 'Arme offensive polyvalente.',
    materials: { wood: 6, ore: 3, resin: 2 },
    result: {
      kind: 'equipment',
      slot: 'weapon',
      baseName: 'Arc du traqueur',
      attack: 15,
      defense: 2,
      value: 190,
      rarity: 'rare',
      weaponType: 'bow',
      icon: '/assets/Weapons/Wood/Wood.png',
    },
  },
  {
    id: 'recipe_bone_dagger',
    name: 'Dague osseuse',
    rarity: 'uncommon',
    description: 'Arme de mêlée rapide.',
    materials: { bone_dust: 4, ore: 2 },
    result: {
      kind: 'equipment',
      slot: 'weapon',
      baseName: 'Dague osseuse',
      attack: 10,
      defense: 1,
      value: 130,
      rarity: 'uncommon',
      weaponType: 'melee',
      icon: '/assets/Weapons/Bone/Bone.png',
    },
  },
  {
    id: 'recipe_bark_armor',
    name: 'Armure d\'écorce',
    rarity: 'rare',
    description: 'Armure polyvalente légère.',
    materials: { wood: 6, resin: 3, herb: 3 },
    result: {
      kind: 'equipment',
      slot: 'armor',
      baseName: 'Armure d\'écorce',
      attack: 2,
      defense: 17,
      value: 230,
      rarity: 'rare',
      icon: '/assets/Weapons/Hands/Hands.png',
    },
  },
  {
    id: 'recipe_ritual_staff',
    name: 'Baton rituel',
    rarity: 'epic',
    description: 'Baton de lanceur polyvalent.',
    materials: { wood: 7, ether_drop: 3, obsidian_fragment: 2 },
    result: {
      kind: 'equipment',
      slot: 'weapon',
      baseName: 'Bâton rituel',
      attack: 21,
      defense: 4,
      value: 300,
      rarity: 'epic',
      weaponType: 'staff',
      icon: '/assets/Weapons/Wood/Wood.png',
    },
  },
  {
    id: 'recipe_bastion_plate',
    name: 'Plastron bastion',
    rarity: 'epic',
    description: 'Armure défensive lourde.',
    materials: { ore: 8, obsidian_fragment: 4, bone_dust: 3 },
    result: {
      kind: 'equipment',
      slot: 'armor',
      baseName: 'Plastron bastion',
      attack: 3,
      defense: 27,
      value: 330,
      rarity: 'epic',
      icon: '/assets/Weapons/Hands/Hands.png',
    },
  },
  {
    id: 'recipe_void_charm',
    name: 'Charme du vide',
    rarity: 'legendary',
    description: 'Relique legendaire de support.',
    materials: { ether_drop: 6, obsidian_fragment: 6, boss_shard: 2 },
    result: {
      kind: 'equipment',
      slot: 'trinket',
      baseName: 'Charme du vide',
      attack: 20,
      defense: 16,
      value: 520,
      rarity: 'legendary',
      icon: '/assets/Icons/anneau.png',
    },
  },
  {
    id: 'recipe_stability_shard',
    name: 'Éclat de stabilité',
    rarity: 'epic',
    description: 'Protège un équipement d\'une rétrogradation lors d\'un échec de forge.',
    materials: { boss_shard: 1, obsidian_fragment: 3, ether_drop: 2 },
    result: {
      kind: 'material',
      material: 'stability_shard',
      quantity: 1,
      icon: '/assets/Icons/stability_shard.svg',
    },
  },
  {
    id: 'recipe_spirit_chisel',
    name: 'Ciseau des esprits',
    rarity: 'rare',
    description: 'Ouvre un nouvel emplacement de pierre d\'esprit sur un équipement, en forge.',
    materials: { ore: 5, obsidian_fragment: 2, bone_dust: 2 },
    result: {
      kind: 'material',
      material: 'spirit_chisel',
      quantity: 1,
      icon: '/assets/Icons/spirit_chisel.svg',
    },
  },
  // ── Recettes de camp (consommables et utilitaires) ────────────────────────
  {
    id: 'recipe_health_potion',
    name: 'Potion de soin',
    rarity: 'common',
    description: 'Récupère 50 PV en combat.',
    category: 'camp',
    materials: { herb: 2, resin: 1 },
    result: {
      kind: 'consumable',
      name: 'Potion de soin',
      effect: 'heal_50',
      icon: '/assets/Icons/life_potion.png',
      quantity: 1,
    },
  },
  {
    id: 'recipe_mana_elixir',
    name: 'Élixir de mana',
    rarity: 'common',
    description: 'Restaure 60 mana en combat.',
    category: 'camp',
    materials: { herb: 2, ether_drop: 2 },
    result: {
      kind: 'consumable',
      name: 'Élixir de mana',
      effect: 'mana_60',
      icon: '/assets/Icons/mana_potion.png',
      quantity: 1,
    },
  },
  {
    id: 'recipe_torch',
    name: 'Torche runique',
    rarity: 'common',
    description: 'Éclaire les zones sombres.',
    category: 'camp',
    materials: { wood: 2, resin: 1 },
    result: {
      kind: 'consumable',
      name: 'Torche runique',
      effect: 'torch',
      icon: '/assets/Icons/torche-runique.png',
      quantity: 1,
    },
  },
  {
    id: 'recipe_campfire',
    name: 'Feu de camp',
    rarity: 'common',
    description: 'Restaure tous les PV et tout le mana au camp.',
    category: 'camp',
    materials: { wood: 3, resin: 2 },
    result: {
      kind: 'consumable',
      name: 'Feu de camp',
      effect: 'campfire',
      icon: '/assets/Icons/firecamp.png',
      quantity: 1,
    },
  },
]

export const CONSUMABLES_SHOP = [
  {
    id: 'shop_potion',
    name: 'Potion de santé',
    price: 40,
    effect: 'heal_50',
    description: 'Rend 50 PV.',
  },
  {
    id: 'shop_mana',
    name: 'Elixir de mana',
    price: 50,
    effect: 'mana_60',
    description: 'Rend 60 mana.',
  },
  {
    id: 'shop_resist_tonic',
    name: 'Tonique de resistance',
    price: 75,
    effect: 'buff_resistance',
    description: 'Augmente la DEF de 24% pendant 3 tours.',
    stock: 2,
  },
  {
    id: 'shop_fury_draft',
    name: 'Fiole de furie',
    price: 77,
    effect: 'buff_damage',
    description: 'Augmente les degats de 24% pendant 3 tours.',
  },
  {
    id: 'shop_focus_oil',
    name: 'Huile de focus',
    price: 82,
    effect: 'buff_crit',
    description: 'Augmente Crit% et Crit DMG pendant 3 tours.',
    stock: 2,
  },
  {
    id: 'shop_recovery_kit',
    name: 'Kit de recuperation',
    price: 100,
    effect: 'heal_45_mana_35',
    description: 'Rend 45 PV et 35 mana.',
  },
  {
    id: 'shop_clarity_orb',
    name: 'Orbe de clarte',
    price: 86,
    effect: 'cleanse_and_guard',
    description: 'Retire les debuffs et confere un bouclier.',
    stock: 1,
  },
  {
    id: 'shop_vision_torch',
    name: 'Torche runique',
    price: 65,
    effect: 'vision_boost',
    description: '+1 case de vision pendant 14 pas.',
    stock: 2,
  },
]

export const RARITY_BONUS_RULES = {
  epic: { min: 1, max: 1 },
  legendary: { min: 1, max: 2, secondChance: 0.36 },
  mythic: { min: 3, max: 3 },
}

export const EQUIPMENT_BONUS_POOL = [
  { id: 'bonus_hp', label: 'PV max', key: 'maxHpFlat', min: 12, max: 34, weight: 14 },
  { id: 'bonus_attack', label: 'Dégâts', key: 'attackFlat', min: 1, max: 5, weight: 11 },
  { id: 'bonus_defense', label: 'Armure', key: 'defenseFlat', min: 1, max: 4, weight: 12 },
  { id: 'bonus_crit', label: 'Chance crit', key: 'critChanceFlat', min: 0.02, max: 0.07, weight: 9, percent: true },
  { id: 'bonus_crit_damage', label: 'Dégâts crit', key: 'critDamageFlat', min: 0.05, max: 0.15, weight: 9, percent: true },
  { id: 'bonus_dodge', label: 'Chance esquive', key: 'dodgeChanceFlat', min: 0.02, max: 0.08, weight: 10, percent: true },
  { id: 'bonus_parry', label: 'Chance parade', key: 'parryChanceFlat', min: 0.02, max: 0.08, weight: 9, percent: true },
  { id: 'bonus_status', label: 'Chance desorienter', key: 'statusChanceFlat', min: 0.03, max: 0.1, weight: 8, percent: true },
  { id: 'bonus_resist', label: 'Résistance états', key: 'statusResistFlat', min: 0.03, max: 0.12, weight: 8, percent: true },
  { id: 'bonus_regen', label: 'Regen PV/tour', key: 'lifeRegenFlat', min: 1, max: 5, weight: 7 },
  { id: 'bonus_mana_regen', label: 'Regen mana/tour', key: 'manaRegenFlat', min: 2, max: 7, weight: 7 },
]

export const LOOT_BASES = {
  weapon: [
    {
      name: 'Lame fendue',
      attack: 6,
      defense: 0,
      value: 60,
      weaponType: 'melee',
      icon: '/assets/Weapons/Hands/Hands.png',
    },
    {
      name: 'Glaive du voile',
      attack: 7,
      defense: 1,
      value: 78,
      weaponType: 'melee',
      icon: '/assets/Weapons/Bone/Bone.png',
    },
    {
      name: 'Fendoir des catacombes',
      attack: 5,
      defense: 1,
      value: 96,
      weaponType: 'staff',
      icon: '/assets/Weapons/Wood/Wood.png',
    },
    {
      name: 'Arc osseux',
      attack: 5,
      defense: 0,
      value: 88,
      weaponType: 'bow',
      icon: '/assets/Weapons/Wood/Wood.png',
    },
    {
      name: 'Griffe d\'os ancien',
      attack: 10,
      defense: 0,
      value: 115,
      weaponType: 'melee',
      icon: '/assets/Weapons/Bone/Bone.png',
      boneWeapon: true,
    },
    {
      name: 'Sceptre des ossements',
      attack: 9,
      defense: 2,
      value: 132,
      weaponType: 'staff',
      icon: '/assets/Weapons/Bone/Bone.png',
      boneWeapon: true,
    },
    {
      name: 'Arc des ancêtres',
      attack: 8,
      defense: 1,
      value: 126,
      weaponType: 'bow',
      icon: '/assets/Weapons/Bone/Bone.png',
      boneWeapon: true,
    },
    {
      name: 'Dague de vide',
      attack: 11,
      defense: 0,
      value: 148,
      weaponType: 'melee',
      icon: '/assets/Weapons/Bone/Bone.png',
      boneWeapon: true,
    },
  ],
  armor: [
    { name: 'Cuir recousu', attack: 0, defense: 4, value: 58, icon: '/assets/Weapons/Hands/Hands.png' },
    { name: 'Plastron noirci', attack: 1, defense: 6, value: 82, icon: '/assets/Weapons/Hands/Hands.png' },
    { name: 'Armure nervuree', attack: 0, defense: 8, value: 110, icon: '/assets/Weapons/Hands/Hands.png' },
    { name: 'Armure de cristal brisé', attack: 1, defense: 11, value: 148, icon: '/assets/Weapons/Hands/Hands.png' },
    { name: 'Cuirasse des ombres', attack: 2, defense: 13, value: 190, icon: '/assets/Weapons/Hands/Hands.png' },
  ],
  trinket: [
    { name: 'Talisman fendu', attack: 2, defense: 2, value: 70, icon: '/assets/Icons/anneau.png' },
    { name: 'Anneau de cendre', attack: 3, defense: 1, value: 84, icon: '/assets/Icons/anneau.png' },
    { name: 'Idole des ruines', attack: 4, defense: 2, value: 122, icon: '/assets/Icons/anneau.png' },
    { name: 'Pendentif de cristal', attack: 5, defense: 4, value: 168, icon: '/assets/Icons/anneau.png' },
    { name: 'Sceau du vide', attack: 6, defense: 5, value: 210, icon: '/assets/Icons/anneau.png' },
    { name: 'Orbe d\'Onyx', attack: 8, defense: 6, value: 280, icon: '/assets/Icons/anneau.png' },
  ],
}

export const EQUIPMENT_SLOTS = ['weapon', 'armor', 'trinket']



